...
class testBlackList(TestCase):
    ...
    def xtest_cumplen_perfil(self):
        endpoint_cumplen='/cumplenPerfil/302'
        solicitud_cumplen=self.client.get(endpoint_cumplen)
        respuesta_cumplen=json.loads(solicitud_cumplen.get_data())
        self.assertEqual(solicitud_cumplen.status_code, 200)
        self.assertEqual(respuesta_cumplen["Respuesta"]["Seleccion"][0]["id_cand"], 504)

    def xtest_consulta_empresa(self):
        endpoint_empresa='/empresa/1'
        solicitud_empresa=self.client.get(endpoint_empresa)
        respuesta_empresa=json.loads(solicitud_empresa.get_data())
        self.assertEqual(solicitud_empresa.status_code, 200)
        self.assertEqual(respuesta_empresa["id"], 1)
